...
//define ball color struct with name of color, threshold for sensing, and color for LED:
...
void setup() {
  ...
  //Make sure color sensor initializes
  if (tcs.begin()) {
...
void loop() {
  ...
  tcs.setInterrupt(true);                             //honestly not sure about this part but it was in example code or somethin           

  //Checking the color of the ball
  ...
  //Printing what was detected to serial monitor
  ...
  //Routing the ball to the correct bin/trash
  [&]{
    for(int i = 0; i<3; i++){                                                                           //for each bin (3 total)
      for(int j = 0; j<4; j++){                                                                         //for each spot in the bin (4 total)
        Serial.println("Bin " + String((i+1)) + ", Item " + String(j+1) + " is " + bin[i].item[j]);     //write what is in that spot (total of 12 spots)
        if(String(bin[i].item[j]) == String(color)){                                                    //if the sensed ball matches one of the colors
          ...
          Serial.println("Bin " + String(i+1) + ", item " + String(j+1) + " done.");                    //display the spot in the bin is complete
          Serial.println("Balls Left to Sort: " + String(ballsToSort));                                 //display # of balls left to sort
          colorBlink(RGBwhite, 3);                                                                      //flash that a ball has been sorted correctly
          return;                                                                                       //exit both for loops by exiting the [&] flag
        }                                       
      }                                         //"else" if the sensed ball does not match any of the required balls
    }                                           //(if the conditional is never true it doesn't leave the [&] flag before this part runs)                      
  ...
  if( (ballsToSort==0) && (ballsTotal == 0)){         //if all balls have been sorted and there are no balls left in the hopper
